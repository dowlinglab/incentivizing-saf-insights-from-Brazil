...
This file contains a function defining a supply chain model for distributing bio-jet fuel capacity
 via the alcohol-to-jet (ATJ) technology in Brazil to support the manuscript "Incentivizing Sustainable Aviation Fuel: Supply Chain
and Policy Insights from Brazil."
...
def create_supply_chain_model(data, saf_prem, blend, max_saf_capacity, profit_obj = True, grass_roots_factor=0.5, breakpoints=10, mill_spef_prem=False):
    '''
    ...
    Inputs: 
            ...
            mill_spef_prem: True if solving for mill specific incentives (SI Section S8)
    ...
    #SETS
    products_and_intermeadiates = ['jui', 'j1', 'j2', 'bag', 'sug', 'et', 'el', 'mol', 'saf', 'etmk', 'etsaf', 'etmill','etr', 'etpc','g','d','blended saf']
    ...
    #Reference Profits for mill-specific incentives (SI Section S8)
    ref_prof1a = {}
    for i in range(len(mills)):
    ...
    m.reference_capex = pyo.Param(initialize = 11970824, mutable = True) #2023 R$/year (amortized)
    ...
    m.x_ref = pyo.Var(m.REFINERIES, m.PRODUCTS_AND_INTERMEADIATES, within=pyo.NonNegativeReals) #Amount of products and intermeadiates produced at each refinery
    #Purchased
    ...
    def ethanol_destinations(m, i):
        return m.x[i,'et'] == m.x[i,'etmk'] + m.x[i,'etmill'] + m.x[i,'etsaf'] + m.x[i,'etr']
    ...
    def ethanol_sold(m, i):
        return m.x[i, 'etmill'] == m.eth_sold_sum[i]
    ...
    def eth_selling_constraint(m,i):
        return m.x[i, 'etmill'] <= (1-m.y[i])*m.M 
    ...
    def saf_sold_ref(m,i):
        return m.x[i,'saf'] == m.saf_sold_ref_sum[i]
    ...
    def ground_transport_demand(m):
        return (m.p['et'] + sum(m.x[i,'etmk'] for i in m.MILLS))*m.ethanol_energy + (m.p['g'] + sum(m.x[i,'g'] for i in m.MILLS) + sum(m.x_ref[j,'g'] for j in m.REFINERIES))*m.gas_energy >= m.ground_demand
    ...
    def SAF_investment_lower(m, i):
        return m.x[i,'saf'] >= m.y[i]*28000 #enforcing a minimum investment size to enforce no investments with negligible production since CAPEX is a small cost contributer
    ...
    def SAF_investment_lower_ref(m,i):
        return m.x_ref[i,'saf'] >= m.y_ref[i]*28000 #enforcing a minimum investment size to enforce no investments with negligible production since CAPEX is a small cost contributer
    ...
    def opex_sum(m):
        return sum(m.cost['sug']*m.x[i,'sug'] + m.cost['et']*m.x[i,'et'] + m.cost['saf']*m.x[i,'saf'] + m.cost['el']*m.x[i,'el'] for i in m.MILLS) + sum(m.greenfield_opex_ref*m.x_ref[j,'saf'] for j in m.REFINERIES)
    ...
    def individual_rev_mill(m,i):
        return m.price['sug']*m.x[i,'sug'] + m.price['et']*(m.x[i,'etmk'] + m.x[i,'etr'] + m.x[i,'etmill']) + m.price['el']*m.x[i,'el'] + (m.price['saf'] + m.saf_premium)*m.x[i,'saf']
    ...
    #Capex
    def capex_sum(m):
        return sum(m.CAPEX[i] for i in m.MILLS) + sum(m.CAPEX_ref[i] for i in m.REFINERIES)
    ...
    #Profit lower bound
    if mill_spef_prem == True:
        #Mills must make as much as max profit with no SAF production
    ...
    def profit_expression(m):
        return m.total_rev - (sum(m.individual_opex_mill[i] for i in m.MILLS) + sum(m.price['et']*m.x[i,'etpc'] for i in m.MILLS) + m.mill_to_mill_logistic_cost + m.mill_to_ref_logistic_cost + sum(m.CAPEX[i] for i in m.MILLS))
    m.profit_expression = pyo.Expression(rule = profit_expression)

    #Total Supply Chain Cost Expression
    def sc_cost_expression(m):
        return m.opex_sum + m.mill_to_mill_logistic_cost + m.mill_to_ref_logistic_cost + m.ref_to_air_logistic_cost + m.capex_sum + m.additional_costs + sum(m.s[i] for i in m.MILLS)
